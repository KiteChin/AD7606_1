...
/* CONVST */		
#define RCC_CONVST 	RCC_AHB1Periph_GPIOC
#define PORT_CONVST	GPIOC
#define PIN_CONVST 	GPIO_Pin_7

/* BUSY */
#define RCC_BUSY 	RCC_AHB1Periph_GPIOA
#define PORT_BUSY 	GPIOA
#define PIN_BUSY 	GPIO_Pin_4
...
void AD7606_Scan(void) 		/* 此函数代码按照时序编写 */
{
	...
	if (BUSY_IS_LOW())	   
    {
			...
			if (BUSY_IS_LOW())
			{
				while(GPIO_ReadInputDataBit(PORT_CS, PIN_CS))
					AD_CS_0(); /* SPI片选 = 0 */
